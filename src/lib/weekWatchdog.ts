...
export interface MissedJob {
   ...
   * on game day, which is exactly why they need watching. */
  job: 'open-registration' | 'close-registration' | 'generate-teams' | 'game-day-email';
  message: string;
...
export function missedJobsFor(session: Session | null, now: Date = new Date()): MissedJob[] {
  ...
  const milestones = getWeeklyMilestones(
    session?.gameDate ?? expectedGameDate,
    session?.gameTime ?? DEFAULT_GAME_TIME,
